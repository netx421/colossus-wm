...
struct colossus_print_status_manager *print_status_manager;
...
static struct wl_signal colossus_print_status;
...
void printstatus(void) { wl_signal_emit(&colossus_print_status, NULL); }
...
void setup(void) {
	...
	setenv("XDG_CURRENT_DESKTOP", "colossus", 1);
	parse_config();
	init_baked_points();
	...
	// 在 setup 函数中
	wl_signal_init(&colossus_print_status);
	wl_signal_add(&colossus_print_status, &print_status_listener);
