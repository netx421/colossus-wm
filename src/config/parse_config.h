...
void parse_config_file(Config *config, const char *file_path) {
	...
	if (file_path[0] == '.' && file_path[1] == '/') {
		...
		if (cli_config_path) {
			...
			snprintf(full_path, sizeof(full_path), "%s/.config/colossus/%s", home,
					 file_path + 1);
		}
...
void parse_config(void) {
	...
	if (cli_config_path) {
		...
		// 构建日志文件路径
		snprintf(filename, sizeof(filename), "%s/.config/colossus/config.conf",
				 homedir);

		// 检查文件是否存在
		if (access(filename, F_OK) != 0) {
			// 如果文件不存在，则使用 /etc/colossus/config.conf
			snprintf(filename, sizeof(filename), "%s/colossus/config.conf",
					 SYSCONFDIR);
		}
	}
